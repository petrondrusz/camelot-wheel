...
frameChroma()                     → 1 frame: chroma[12], w (tonalness), bass, dB
   ...
   │                                                    │
   ├─► chordChroma EMA (0.45s) ─► detectChord() ─► updateChord() ─► chordTime[], bassTime[]
   │                                                    │           (leaky tally, MEM 20s)
   ▼                                                    ▼
...
4. **Bas:** nejsilnější peak v **55–260 Hz** → pitch class basu (`fr.bass`).
   Nese toniku, používá se v `detectChord` (kotvení) i v `correctKey` (rozhodčí
   kvint). Když je v base ticho, `bass = −1`.
...
| `chordTime[]`, `bassTime[]` | `CHORD_MEM_TAU` 20 s | **nasčítaná paměť** akordů/basu (leaky, ať nová píseň přebije starou) |
...
`chordTime[]` a bas do `bassTime[]` (obojí leaky, paměť 20 s).

`chordTime[]` a `bassTime[]` jsou **vstup pro všechny korekce** níž.
...
### 8a. `correctKey(n0, raw)` — kvintová korekce (keyFit + bas)
...
- **keyFit(m):** kolik času se v `chordTime[]` strávilo na **diatonických
  ...
  akordová progrese sedí na tóninu m".
- **Bas-rozhodčí:** `+BASS_KEY_W (8) · podíl basu na tonice m` (dur i rel. moll
  root). Bas skoro vždy hraje toniku → rozlomí C↔G, co keyFit sám neutáhne.
  Hradlo: jen při `>2 s` nasčítaného basu.
- **Plauzibilitní hradlo:** kandidát `m` se zváží **jen když je chroma
  konkurenční** (`raw[m] ≥ raw[n0] − 0.06`). Jasný chroma favorit = žádná
  záměna, nepřepisuj.
- **Override** jen když `keyFit(best)+bas > 1.2 × (keyFit(n0)+bas)`.
...
- Jinak override jen při jasné převaze (`> 1.3×`).
...
locked = !changing                              // neprobíhá změna songu
       ...
       && (loopSeen || analyzedMs ≥ 30 s)       // progrese udělala smyčku (nebo fallback)
       && corr ≥ CONF ± 0.06                     // jistota (hystereze: zamčené drží snáz)
```
...
ring  locked:min   v9                    ← stav obroučky (searching/locked + mód) · verze buildu
```
...
`APP_VERSION` (`v9`) páruje screenshot s konkrétním buildem — **bump při každém
deployi**.
...
| `CHORD_MEM_TAU` | 20 s | paměť `chordTime`/`bassTime` |
...
| `BASS_KEY_W` | 8 | váha bas-toniky v korekci kvint |
...
| Oasis – Don't Look Back in Anger | 8B (C) | C↔G kvinta (bas-rozhodčí) |
