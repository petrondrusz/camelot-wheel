...
RESOLUCE (analyzeChroma):
   ...
   ▼
   latch   = po zamčení drž lockFav, pusť jen při potvrzené změně písně (§9)
   ▼
...
**Keep-alive (v14) — KRITICKÉ pro Safari/iOS:** AudioContext se sám **suspenduje**
(šetření energií na desktopu, „interrupt" na iOS při notifikaci / Control Center /
jiné audio session). Pak analyser vrací **samé ticho** a overlay ukazuje narůstající
`silence … — restarted`, i když hudba hraje. Proto se kontext **aktivně probouzí**:
`wakeAudio()` (idempotentní `resume()`) se volá každý frame ve `loop()`, na
`audioCtx.onstatechange` a na `visibilitychange`/`focus`. Navíc když iOS **ukončí
mic track**, `onended` znovu otevře stream (`openMicStream()`) — jinak by appka
zůstala trvale hluchá. Diagnostika: spodní VU sloupce se musí hýbat při zvuku; když
jsou ploché a kontext běží, jde o HW/routing (mic neslyší song), ne o appku.
...
- **Latch (v13) — zamčená odpověď drží:** appka je nástroj pro **nehudebníka**,
  co chce rychle JEDNU tóninu k jamování. Proto se favorit **po zamčení zalatchuje**
  (`lockFav`/`lockMode`) a hub i hmatník ukazují **drženou** tóninu, ne živého
  favorita, co bloudí přes modální/kompatibilní sousedy. Latch se pustí jedině při
  **potvrzené změně písně** (viz níž) → pak se detekuje načisto. Důsledek: Hotel
  California chytne na začátku čistý Bm (10A) a **drží ho** přes dorské pasáže,
  místo aby ujelo na 12B (E dur). (Pozn.: to vědomě přebíjí i sebevědomější chroma
  živého framu — §0 platí pro *detekci*, latch je vrstva *stability* nad ní.)
- **Změna songu = vzdálený favorit + zhroucení domácí tóniny:** `changing` se
  spustí, jen když krátkodobý `heat` je **harmonicky vzdálený** (`camDist > 1`,
  tj. ne relativní/kvinta) **A ZÁROVEŇ** zamčená tónina ztratila vlastní oporu
  (`slow.raw[lockFav] < CONF_LOCK_MIN` = 0.4). To je rozdíl mezi **vnitropísňovou
  odbočkou** (HC dorská část: domácí Bm má pořád skoro všechny noty → `raw` drží)
  a **novou písní** (noty staré tóniny zmizí → `raw` se zhroutí). Bez téhle druhé
  brány tonálně bohaté songy „lockovaly na pár vteřin a zase hledaly". Po `changing`
  se `keyEma` přeladí rychle (`KEY_TAU_FAST`) a latch padá.
...
ring  locked:min   v14                   ← stav obroučky (searching/locked + mód) · verze buildu
```
...
`APP_VERSION` (`v14`) páruje screenshot s konkrétním buildem — **bump při každém
deployi**.
...
- **Modální směs** (Eagles „Hotel California" = H moll 10A): song **současně
  ...
  **Teoreticky** nelze určit „tu jednu pravdu" (tělo koreluje s dorskou stranou
  silněji, 12B r0.76, než s nat. moll) — to je dané, kategorie SNA. **Prakticky**
  to ale řeší **latch (v13)**: appka chytne na začátku čistý Bm (10A) a **drží ho**
  přes dorské pasáže — protože domácí Bm má i v nich skoro všechny noty, takže se
  jeho `raw` nezhroutí pod 0.4 a latch se nepustí. Pro nehudebníka, co chce jednu
  použitelnou tóninu k jamování (B moll pentatonika sedí na celý song), je to přesně
  správná odpověď. (Globální neleaky akumulátor by 10A taky udržel, ale rozbil by
  detekci změny písně — latch dosáhne stejného bez té oběti.)

U čistě modálních případů (Feeling Good, SNA) bereme: *root/tonální centrum
spolehlivé, přesný mód best-effort* — principiální fix (mixolydian profil) by
zaváděl nové záměny (A-mix a D dur mají identické noty) a riskoval čisté songy.
...
| `CONF_LOCK_MIN` | 0.4 | uvolněný práh zámku po 30 s + práh „zhroucení" zamčené tóniny (latch) |
...
| Eagles – Hotel California | 10A (Bm) | latch: chytí úvodní Bm a drží ho přes dorské pasáže |
