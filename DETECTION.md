...
- **Změna songu:** když krátkodobý odhad (`heat`) **harmonicky vzdálený**
  (`camDist(heat.fav, settled) > 1` — ne relativní/kvinta) odporuje ustálenému
  déle než `CHANGE_MS` (4 s), `keyEma` se přeladí rychle (`KEY_TAU_FAST`). Kolísání
  mezi **Camelot-kompatibilními** sousedy (relativní = stejné číslo, kvinta = ±1) je
  normální vnitropísňový pohyb, ne nový song — bez téhle brány tonálně bohaté písně
  „lockovaly na pár vteřin a zase hledaly". (Změna *do* kompatibilní tóniny se
  schválně nehlásí — není urgentní, pomalý integrátor tam doplave.)
  > **Slepá ulička (v13, revertováno):** zkusili jsme „latch" — po zamčení DRŽET
  > favorita a pustit jen při zhroucení domácí tóniny. Mělo to udržet úvodní 10A
  > u Hotel California. **Backfire:** latch zmrazil i *chybný* raný lock u písní,
  > co se dřív samy opravily, takže regredoval i songy, kde appka fungovala. Klasická
  > výměna adaptivity za stabilitu — pro jeden sporný modální song nestála za obecnou
  > regresi. Revertováno (v15). Lekce: stabilizační vrstva nesmí přebít schopnost
  > opravit ranní omyl. (Bezpečnější „soft latch" — ustoupit setrvale silnějšímu
  > usazenému čtení — by šel, ale jen pokud nerozbije etalon.)
...
ring  locked:min   v17                   ← stav obroučky (searching/locked + mód) · verze buildu
```
...
`APP_VERSION` (`v17`) páruje screenshot s konkrétním buildem — **bump při každém
deployi**.
...
- **Modální směs** (Eagles „Hotel California" = H moll 10A): song **současně
  ...
  **Nelze určit „tu jednu pravdu"** (kategorie SNA): tělo koreluje s dorskou
  stranou *silněji* (12B r0.76) než s nat. moll, takže chroma se usadí podle pasáže.
  Zkoušeli jsme to vynutit „latchem" (v13) — držet úvodní 10A — ale ten regredoval
  obecné songy (zmrazil i chybné rané locky), tak jsme ho revertovali (viz §9 slepá
  ulička). Pro jamování to vadí míň, než vypadá: 10A, 11B, 11A i 9A jsou vzájemně
  **Camelot-kompatibilní** a B moll pentatonika sedí na celý song — ať appka ukáže
  kteroukoli z nich, hraje se stejně.
