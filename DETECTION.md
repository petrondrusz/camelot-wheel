...
- **Změna songu:** když krátkodobý odhad (`heat`) jistě odporuje ustálenému
  ...
  „Odporuje" se počítá jen při **harmonicky vzdáleném** favoritovi —
  `camDist(heat.fav, settled) > 1`. Kolísání mezi **Camelot-kompatibilními**
  sousedy (relativní = stejné číslo, kvinta = ±1) je normální vnitropísňový pohyb,
  ne nový song. Bez téhle brány tonálně bohaté písně „lockovaly na pár vteřin a
  zase hledaly" (Hotel California ujíždí Bm↔F#m↔D = 10A↔11A↔10B — vše kompatibilní).
  Teď zámek **drží** a popisek se zpřesňuje na místě. (Změna *do* kompatibilní
  tóniny se schválně nehlásí — není urgentní, pomalý integrátor tam doplave.)
...
ring  locked:min   v12                   ← stav obroučky (searching/locked + mód) · verze buildu
```
...
`APP_VERSION` (`v12`) páruje screenshot s konkrétním buildem — **bump při každém
deployi**.
